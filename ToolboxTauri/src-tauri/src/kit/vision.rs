...
pub fn blur_faces(request: &VisionRequest, input: PathBuf) -> JobOutcome {
    run_image_adapter(request, input, "faceBlur", "TOOLBOX_FACE_BLUR_PATH", "toolbox-face-blur", "-blurred", "Blurred faces", false)
}

pub fn remove_background(request: &VisionRequest, input: PathBuf) -> JobOutcome {
    run_image_adapter(request, input, "backgroundRemoval", "TOOLBOX_BACKGROUND_REMOVAL_PATH", "toolbox-background-removal", "-cutout", "Background removed", true)
}

fn run_image_adapter(request: &VisionRequest, input: PathBuf, resource_name: &str, variable: &str, command: &str, suffix: &str, detail: &str, validate_alpha: bool) -> JobOutcome {
    ...
    match Command::new(engine).arg(&input).arg(&output).output() {
        Ok(result) if result.status.success() && output.is_file() => {
            if validate_alpha { match validate_cutout(&output) { Ok(()) => success(input, output, detail), Err(error) => { let _ = std::fs::remove_file(&output); failure(input, error) } } }
            else { success(input, output, detail) }
        }
